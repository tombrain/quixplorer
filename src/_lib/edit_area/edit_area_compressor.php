...
class Compressor
{
	...
	function load_files()
	{
		...
		$loader = preg_replace_callback(
			...
			function ($m)
			{
				return $this->replace_scripts('script_list', $m[1], $m[2]);
			},
			$loader
		);
		...
		$loader = preg_replace_callback(
			...
			function ($m)
			{
				return $this->replace_scripts('sub_script_list', $m[1], $m[2]);
			},
			$loader
		);
		...
		$this->datas .= sprintf(
			"editAreaLoader.iframe_script= \"<script type='text/javascript'>%s</script>\"%s;\n",
